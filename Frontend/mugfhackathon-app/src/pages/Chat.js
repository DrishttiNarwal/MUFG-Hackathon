...
export default function Chat() {
  const [mode, setMode] = useState(null); // null → choose, "form" → form UI, "chat" → chatbot
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Collected Data (Form): ", formData);
    alert("Form Submitted! Check console for data.");
  };

  // ✅ Data collected from chatbot
  const handleChatComplete = (answers) => {
    console.log("Collected Data (Chatbot): ", answers);
    alert("Chat Completed! Check console for data.");
  };
    ...
    <div className="chat-page">
      ...
      {mode === null && (
        <div className="mode-selector">
          <button onClick={() => setMode("form")} className="mode-btn">
            Fill Form
          </button>
          <button onClick={() => setMode("chat")} className="mode-btn alt">
            Talk to Chatbot
          </button>
        </div>
      )}
        ...
        <div className="chat-container-wrapper">
          <form className="form-container" onSubmit={handleSubmit}>
            ...
            {formData.productType === "health" || formData.productType === "life" ? (
              <>
                <h2>
                  {formData.productType === "health" ? "Health Insurance" : "Life Insurance"} Details
                </h2>
                <input
                  type="number"
                  name="annualPremium"
