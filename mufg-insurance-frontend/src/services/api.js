...
const API_BASE_URL = process.env.REACT_APP_API_URL || (isDevelopment ? 'http://localhost:8000' : 'http://13.200.250.84:8000');
...
export const insuranceService = {
  ...
  getRecommendations: async (policyType, country, formData) => {
    try {
      ...
      // Convert smokerdrinker to proper format
      if (requestData.smokerdrinker) {
        requestData.smokerdrinker = requestData.smokerdrinker === 'yes' ? 'Yes' : 'No';
      }
